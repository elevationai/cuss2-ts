...
// Line 1: Document type (P), Issuing country (USA), Name (TESTER<<TEST)
// Line 2: Passport number, Nationality, DOB, Sex, Expiry, Personal number, Check digits
const testPassportMRZ = 'P<USATESTER<<TEST<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n5123456789USA9001014M3012315<<<<<<<<<<<<<<04';

// Sample boarding pass SEND data (for boarding pass printer)
const sampleBoardingPass = 'CP#A#01S#CP#C01#02@@01#03M1TEST/ADULT                                            RFIFWX    DENLASF9    0775    174Y014B0004    347>5181        1174BF9    042231015000129000000000000000                                                                                                        #04TEST/ADULT#05WED,    JUN    23,    2021#06#07SEQ004#08#09#12RFIFWX#13NO    CARRY    ON    ALLOWED#14#15#16#17#20DEN    -->    LAS#30Denver    to    Las    Vegas#32F9        775#3312:30AM#3401:15PM#3501:00PM#404#43#4414B#5092518095#54#55#64Sold    by#66Frontier    Airlines#';

// Sample bag tag SEND data (for bag tag printer)
const sampleBagTag = 'BTP010101#0101#02SCOTT    SABLAN#03    19    #04        22        #05HARMONY#07BAG    1    OF    2#082345#0A    01JUL    17#0BSAT-MIA#';

const aeaCommands = {
  // Commands for printers (printers use setup/send with SEND data)
  boardingPassPrinter: {
    'Sample Boarding Pass': sampleBoardingPass
  },
  bagTagPrinter: {
    'Sample Bag Tag': sampleBagTag
  },
  // Test data for barcode readers (for testing read functionality)
  barcodeReader: {
    'Test Boarding Pass Barcode': testBarcode
  },
  // Test data for document readers (for testing read functionality)
  documentReader: {
    ...
    try {
      ...
      if (inputValue) {
        if (action === 'read') {
          ...
          // The input should be the raw data string (e.g., ITPS command for printers)
          // Wrap it in a DataRecord array with proper format
          data = [{
            data: inputValue,
