...
/**
 * Format a timestamp as a relative time string (e.g., "now", "30 seconds ago", "10 minutes ago")
...
// Track data timestamps for relative time updates
const dataTimestamps = new Map();

/**
 * Update all visible data timestamps with relative time
 */
function updateAllDataTimestamps() {
  dataTimestamps.forEach((timestamp, componentId) => {
    ...
    const timestampEl = componentItem.querySelector('.data-timestamp');
    if (timestampEl) {
...
setInterval(updateAllDataTimestamps, 5000);
...
const templates = {
  ...
  populateActionColumns(leftColumn, rightColumn, id, component, capabilities) {
    ...
    if (capabilities.includes('send')) {
      ...
      // Media input components (barcode reader, document reader, etc.)
...
const componentHandlers = {
  ...
  updateDataDisplay(componentId, dataRecords) {
    ...
    // Update timestamp for data freshness indication
    const now = Date.now();
    dataTimestamps.set(componentId, now);
    const timestampEl = componentItem.querySelector('.data-timestamp');
    if (timestampEl) {
